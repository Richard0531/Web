...
def image(request):
    ...
    def parse_contents(contents, filename):
        ...
        return df
    def parse_image_contents(contents, filename):
        content_type, content_string = contents.split(',')
        decoded = base64.b64decode(content_string)
    ...
    def update_output(contents_data, filename_data):
        if contents_data is not None:
            df = parse_contents(contents_data, filename_data)
    ...
    @app.callback(
        Output("imshow-plot", 'figure'),
        Input('datatable', 'data'),
        State('upload-image', 'contents'),
        State('upload-image', 'filename'),
        )
    def plot_graph(data,img_contents,img_filename):
        if data is not None and img_contents is not None:
            df =  pd.DataFrame(data)
            fig = px.imshow(df.pivot('Row', 'Column', 'color'),color_continuous_scale="Blues")
            img_filename = parse_image_contents(img_contents,img_filename)
